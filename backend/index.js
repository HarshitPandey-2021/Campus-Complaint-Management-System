...
// ✅ FIXED: Upload to Cloudinary Helper with better error handling
...
// ✅ NEW: Proxy route to serve PDFs with correct headers
app.get('/api/files/pdf/:publicId', async (req, res) => {
  try {
    ...
    // Send with correct headers for browser viewing
...
// ✅ FIXED: Complaints Create with File Upload
app.post('/api/complaints', auth, upload.fields([
  ...
  try {
    ...
    // ✅ FIXED: Upload PDF to Cloudinary with raw type
    let pdfUrl = null
    ...
    const complaint = {
      ...
      images: imageUrls,
      pdfDocument: pdfUrl,
      pdfPublicId: pdfPublicId,  // ✅ Store public ID for proxy access
      status: 'Pending',
      assignedTo: null,
      adminRemarks: '',
...
// ✅ FIXED: Update complaint with file uploads
app.put('/api/complaints/:id', auth, upload.fields([
  { name: 'images', maxCount: 5 },
  { name: 'pdfDocument', maxCount: 1 }
]), async (req, res) => {
  try {
...
app.get('/api/complaints', auth, async (req, res) => {
  try {
    const c = await Complaints.find().sort({ submittedAt: -1 }).toArray()
    res.json(c)
  } catch {
    res.status(500).json({ message: 'Internal server error' })
  }
...
// Bulk update complaints status (Admin)
...
// ✅ NEW: Get files list from Cloudinary (for debugging)
...
// ✅ NEW: Delete file from Cloudinary (Admin only)
