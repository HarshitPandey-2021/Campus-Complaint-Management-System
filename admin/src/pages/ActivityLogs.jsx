...
const ActivityLogs = () => {
  ...
  const loadLogs = () => {
    ...
    setTimeout(() => {
      try {
        const allLogs = getAllLogs(token);
        setLogs(allLogs);
  ...
  const handleClearAll = () => {
    if (window.confirm("Are you sure? This will delete all activity logs.")) {
      clearAllLogs();
      loadLogs();
      success("All activity logs cleared");
    }
  };
  ...
  const getActivitySummary = (log) => {
    ...
    switch (log.type) {
      ...
      case ACTIVITY_TYPES.COMPLAINT_EXPORT:
        return `${details.action} - Exported ${details.complaintCount || 0} items`;
      case ACTIVITY_TYPES.LOGIN:
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-base sm:text-lg font-bold text-gray-800 dark:text-gray-200">
            {filteredLogs.length}{" "}
            {filteredLogs.length === logs.length ? "logs" : `of ${logs.length} logs`}
          </h3>
        </div>
            ...
            <div className="p-4 sm:p-6 space-y-4">
              ...
              <div>
                ...
                <div className={`inline-flex items-center gap-2 mt-1 px-3 py-1 rounded-lg ${getActivityColor(selectedLog.type)}`}>
                  ...
                  <span className="font-medium text-sm">{selectedLog.type}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg text-xs">
                  <p className="text-gray-600 dark:text-gray-400 mb-1 break-all">
                    ...
                    {selectedLog.device?.platform || "N/A"}
                  </p>
                  <p className="text-gray-600 dark:text-gray-400 mb-1 break-all">
                    <span className="font-medium">Browser:</span>{" "}
                    {selectedLog.device?.userAgent || "N/A"}
                  </p>
                  <p className="text-gray-600 dark:text-gray-400 break-all">
                    <span className="font-medium">Screen:</span>{" "}
                    {selectedLog.device?.screenSize || "N/A"}
                  </p>
                </div>
              </div>
