...
export default function ComplaintDetailsPage() {
  ...
  useEffect(() => {
    ...
    getComplaintById(id, token).then(setComplaint);
  }, [id]);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-4xl mx-auto p-6 mt-10">
        <Link to="/my-complaints" className="text-sm px-4 py-2 rounded-full shadow bg-white border hover:bg-gray-50">
          ← Back to My Complaints
        </Link>
        <div className="bg-white rounded-2xl shadow-xl p-10 mt-6 relative">
          {/* Use complaint.title/description/status/etc */}
          <div
            className="absolute top-0 left-0 w-full h-2 rounded-t-2xl"
            style={{background: "linear-gradient(90deg,#c026d3,#ec4899,#0ea5e9,#008080)"}}
          />
          ...
          <div className="space-y-4">
            {(complaint.updates || []).map((u, index) => (
              <div key={index} className="bg-gray-50 p-4 rounded-xl border shadow-sm">
                <p className="font-semibold text-indigo-600">{u.by} • {u.date}</p>
